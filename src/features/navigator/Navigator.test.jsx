...
it('opens a source-driven Call Guide with safe copy actions before lossless source views', async () => {
  ...
  expect(within(dialog).getByRole('button', { name: 'Copy Current Path' })).toBeInTheDocument()
  expect(within(dialog).getByRole('button', { name: /Resolved \/ Done/i })).toBeInTheDocument()
  expect(within(dialog).getByRole('button', { name: /Not resolved/i })).toBeInTheDocument()
  expect(within(dialog).getByText(/Step 1 of/i)).toBeInTheDocument()
