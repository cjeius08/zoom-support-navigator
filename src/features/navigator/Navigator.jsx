...
export function Navigator({ onFeedback }) {
  ...
  return <main className="navigator">
    <section className="hero"><p className="eyebrow">Live support toolkit</p><h1>Find the next step <em>without opening documents.</em></h1><p>Search what the customer is experiencing, then follow a short visual process.</p><input aria-label="Search support processes" value={query} onChange={e => { setQuery(e.target.value); setCategory(null) }} placeholder="Try: can’t hear, camera black, waiting for host…" /></section>
