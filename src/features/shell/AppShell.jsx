...
export function AppShell({ profile, children, onLogout, onAvatarChange }) {
  ...
  return <div className="app-shell">
    <header className="app-header"><button className="menu-toggle" aria-label="Toggle navigation" onClick={() => setOpen(!open)}>☰</button><div className="console-brand"><strong>Zoom</strong><span>Support Console</span></div><div className="header-context">Support workspace</div><button className="account-menu" aria-label={`${profile.username} account`} onClick={() => setProfileOpen(true)}>{avatarUrl(profile.avatar_id) ? <img src={avatarUrl(profile.avatar_id)} alt="" /> : <span className="avatar-fallback">{profile.initials}</span>}<span>{profile.initials}</span><span>{profile.username}</span></button></header>
    <aside className={`sidebar ${open ? 'open' : ''}`}><div><p className="sidebar-label">Workspace</p><nav aria-label="Primary navigation"><a href="#navigator">Navigator</a>{isAdmin && <><a href="#admin">Admin Home</a><a href="#team">Team Management</a><a href="#usage">Usage Analytics</a><a href="#feedback">Feedback</a></>}</nav></div><button className="logout-button" onClick={onLogout}>Logout</button></aside>
